...
# N太大的话csv爆炸无敌大，最好别算shape，小N要shape的几个时间点滑下去改
# enter your csv file name here:
CSV_FILE = "16_alpha0.25_A1_dt0.12_mtp10_0302_2022.csv"
...
my_dpi = 800
important_modes = 5
window_size = 100  # number of points taken to calculate average
scale = 300
...
for i, col in enumerate(modes_cols):  # use enumerate to give i a value
    ...
    alpha_p = 1.0
    line, = plt.plot(t_thousands, df[col], label=col, linewidth=1.5, alpha=alpha_p)
    peak_idx = df[col].idxmax()  # find peak
    peak_x = t_thousands[peak_idx]
    peak_y = df[col][peak_idx]
    if i == 0 and peak_x == t_thousands.min():  # avoid M1 & 1e-7 overlap
        h_align = 'left'
        x_offset = 10
    elif i >= 9:
        h_align = 'center'
...
plt.xlabel("t in thousands of cycles", fontsize=12)
plt.ylabel("mode energy $E_k(t)$", fontsize=12)
# plt.title(f"Energies:{CSV_FILE}")
plt.title(f"FPUT {params['Model'].capitalize()} Model: "
          f"$N={params['N']}$, $\\alpha={params['Alpha_Beta_Value']}$, $A={params['Amplitude']}$",
          fontsize=14, pad=20)
...
plt.plot(t_thousands, rel_error, color='black', linewidth=1.0)
plt.axhline(0, color='red', linestyle='--', alpha=0.5)
plt.xlabel("t in thousands of cycles", fontsize=12)
plt.ylabel(r"Relative Error $\Delta E / E_0$", fontsize=12)
# plt.title(f"Drift:{CSV_FILE}")
plt.title(f"FPUT {params['Model'].capitalize()} Model: "
          f"$N={params['N']}$, $\\alpha={params['Alpha_Beta_Value']}$, $A={params['Amplitude']}$",
          fontsize=14, pad=20)
...
for i, col in enumerate(modes_cols):  # use enumerate to give i a value
    smooth_y = df[col].rolling(window=window_size, center=True).mean()
    ...
    plt.plot(t_thousands, df[col], color=color, alpha=0.1, linewidth=0.5, zorder=1)  # original rough
    plt.plot(t_thousands, smooth_y, color=color, linewidth=lw, alpha=alpha, label=label, zorder=2)  # smooth
...
plt.ylim(0, df[modes_cols].max().max() * 1.1)  # add margin above to avoid overlap
plt.legend(loc='upper right', fontsize=10, framealpha=0.8)
...
if len(x_cols) > 0:
    ...
    plt.title(f"Actual Shape Evolution: FPUT {params['Model'].capitalize()} Model\n"
              f"$N={params['N']}$, $\\alpha={params['Alpha_Beta_Value']}$, $A={params['Amplitude']}$",
              fontsize=14, pad=15)
    ...
    print("No displacement data (x_cols) found. Skipping Actual Shape plot.")
